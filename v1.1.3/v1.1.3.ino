...
// The functionality of this code is unknown. This code is only checked in to
// the repository for archival reference.
...
#define RIBBON_POT_PIN 41  // A0 // 41
...
long unsigned int ribbonPressNextTimeClick =
    0;  // -1 not pressed, otherwise time in ms to
        // delay until listening for first stable value
...
void loop() {
  ...
  ribbonPotVal = (analogRead(RIBBON_POT_PIN) + (-(1 - analogRead(40)))) / 2;
  // Serial.println(ribbonPotVal);

  // Serial.println(ribbonPotVal);
  mappedRibbonPotVal = mapPotVal(ribbonPotVal, 500, 1000, 0, 12);
  // Serial.println(baseFreq * pow(2, mappedRibbonPotVal / 12.0));

  if (mappedRibbonPotVal > -1) {
    // ribbon pressed
    if (ribbonPressNextTimeClick == 0) {
      // initialize delay, wait 10 ms before listening for stable value
      ribbonPressNextTimeClick = timer + 10;
    } else {
      if (ribbonPressNextTimeClick >= timer) {
        ribbonPotVal =
            (analogRead(RIBBON_POT_PIN) + (-(1 - analogRead(40)))) / 2;
        mappedRibbonPotVal = mapPotVal(ribbonPotVal, 500, 1000, 0, 12);
        if (mappedRibbonPotVal > -1) {
          if (curSustainStatus) {
            // sustain note
            if (isStablePotVal(mappedRibbonPotVal, 50)) {
            ...
            // waveform1.frequency(baseFreq * pow(2, mappedRibbonPotVal
            // / 12.0));
            Serial.println(mappedRibbonPotVal);
            setStableVal(mappedRibbonPotVal);
          }
        }
      }
    }
 ...
 * newMin and newMax
 */
int mapPotVal(int potVal, int min, int max, int newMin, int newMax) {
...
int stableLastFourPotVals[4] = {0, 0, 0, 0};
int stableLastFourPotValsIndex = 0;
long unsigned int stableNextTimeClick = 0;
bool isStablePotVal(int potVal, int timeDelta) {
  // check if we have passed the time click
  if (timer >= stableNextTimeClick) {
    // update the last four pot vals
    stableLastFourPotVals[stableLastFourPotValsIndex] = potVal;
    // increment index and wrap around if necessary
    stableLastFourPotValsIndex = (stableLastFourPotValsIndex + 1) % 4;

    // get average of last four pot vals
    int avg = 0;
    for (int i = 0; i < 4; i++) {
      avg += stableLastFourPotVals[i];
    }
    avg /= 4;

    // update the time click
    stableNextTimeClick = timer + timeDelta;
...
void setStableVal(int potVal) {
  for (int i = 0; i < 4; i++) {
    stableLastFourPotVals[i] = potVal;
  }
  stableLastFourPotValsIndex = 0;
}
